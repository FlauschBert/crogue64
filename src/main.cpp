...
void drawText(uint8_t posX, uint8_t posY, char const* text, uint8_t const size, uint8_t const color) {
    volatile uint8_t* screenPtr = (volatile uint8_t*)SCREEN_MEMORY;
...
void drawToScreen(const uint8_t* src, uint8_t const srcWidth, uint8_t const srcHeight,
                  uint8_t const srcOffsetX, uint8_t const srcOffsetY, uint8_t const screenX,
                  uint8_t const screenY, uint8_t const rectWidth, uint8_t const rectHeight,
    ...
    uint8_t screenXEndClamp = clamp(screenX + rectWidth - 1, 0, SCREEN_WIDTH - 1);
    uint8_t screenYEndClamp = clamp(screenY + rectHeight - 1, 0, SCREEN_HEIGHT - 1);
    
    if (screenXClamp > screenXEndClamp || screenYClamp > screenYEndClamp) {
        return;
    }

    uint8_t screenWidthClamped = screenXEndClamp - screenXClamp + 1;
    uint8_t screenHeightClamped = screenYEndClamp - screenYClamp + 1;

    uint8_t srcOffsetXAdjusted = srcOffsetX + (screenXClamp - screenX);
    uint8_t srcOffsetYAdjusted = srcOffsetY + (screenYClamp - screenY);
    ...
    for (uint8_t y = 0; y < screenHeightClamped; ++y) {
        for (uint8_t x = 0; x < screenWidthClamped; ++x) {
            ...
            uint8_t srcX = x + srcOffsetXAdjusted;
            uint8_t srcY = y + srcOffsetYAdjusted;
            uint16_t srcPos = srcY * (uint16_t)srcWidth + srcX;

            if (srcPos >= srcWidth * srcHeight || srcPos < 0) {
                continue;
            }

            uint8_t screenXPos = x + screenXClamp;
            uint8_t screenYPos = y + screenYClamp;
            uint16_t screenPos = screenYPos * (uint16_t)SCREEN_WIDTH + screenXPos;
...
int main() {
    ...
    for (uint8_t i = 0; i < 100; ++i) {
        uint8_t rectWidth = 5 + (rand() % 16);
        uint8_t rectHeight = 5 + (rand() % 11);
        uint8_t screenX = rand() % (SCREEN_WIDTH - rectWidth);
        uint8_t screenY = rand() % (SCREEN_HEIGHT - rectHeight);
